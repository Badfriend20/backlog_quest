...
export function BacklogQuestApp({
  ...
  function openMissionRelation(missionId: string, kind: GameEditorMissionIntent["kind"]) {
    ...
    if (kind === "playthrough" && (!game.copies.length || !game.contents.length)) {
      commands.notify("Agrega una copia y un contenido antes de crear una partida.");
      return;
    }
    setCreatingGame(false);
  ...
  function resolveMissionRelation(
    ...
    commands.commit(
      ...
      relation.kind === "copy"
        ? "Copia agregada y vinculada a la misión."
        : "Partida agregada y vinculada a la misión."
    );
    setGameEditorIntent(undefined);
    setSelectedGameId(null);
  }
          ...
          <GameEditor
            ...
            onRemoveContent={(gameId, contentId) =>
              commands.commit(
                ...
                "Contenido eliminado; misiones y partidas conservaron su descripción histórica."
              )
            }
            onRemoveCopy={(gameId, copyId) =>
              commands.commit(
                current => removeCopy(current, gameId, copyId),
                "Copia eliminada; las misiones y partidas vinculadas permanecen desacopladas."
              )
            }
            onRemovePlaythrough={(gameId, playthroughId) =>
              commands.commit(
                current => removePlaythrough(current, gameId, playthroughId),
                "Partida eliminada; las misiones vinculadas permanecen sin partida."
              )
            }
          />
        )}
